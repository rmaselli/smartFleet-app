  ...
  try {
    ...
    try {
      ...
      const [columns] = await connection.execute(`
        ...
        WHERE TABLE_SCHEMA = 'FLVEHI' 
        AND TABLE_NAME = 'FLVEH_F001' 
        AND COLUMN_NAME IN ('tipo_foto', 'nombre_archivo', 'tamano_archivo', 'tipo_mime')
      `);
      
      const existingColumns = columns.map(col => col.COLUMN_NAME);
      
      if (!existingColumns.includes('tipo_foto')) {
        await connection.execute(`
          ALTER TABLE FLVEHI.FLVEH_F001 
          ADD COLUMN tipo_foto ENUM('lateral_derecha', 'lateral_izquierda', 'frontal', 'trasero', 'odometro') NOT NULL AFTER estado
        `);
        console.log('  ✅ Agregada columna tipo_foto');
      }
      
      if (!existingColumns.includes('nombre_archivo')) {
        await connection.execute(`
          ALTER TABLE FLVEHI.FLVEH_F001 
          ADD COLUMN nombre_archivo VARCHAR(255) AFTER tipo_foto
        `);
        console.log('  ✅ Agregada columna nombre_archivo');
      }
      
      if (!existingColumns.includes('tamano_archivo')) {
        await connection.execute(`
          ALTER TABLE FLVEHI.FLVEH_F001 
          ADD COLUMN tamano_archivo INT AFTER nombre_archivo
        `);
        console.log('  ✅ Agregada columna tamano_archivo');
      }
      
      if (!existingColumns.includes('tipo_mime')) {
        await connection.execute(`
          ALTER TABLE FLVEHI.FLVEH_F001 
          ADD COLUMN tipo_mime VARCHAR(50) AFTER tamano_archivo
        `);
        console.log('  ✅ Agregada columna tipo_mime');
      }
    ...
    try {
      ...
      const [columns] = await connection.execute(`
        ...
        FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_SCHEMA = 'FLVEHI' 
        AND TABLE_NAME = 'FLVEH_F002' 
        AND COLUMN_NAME IN ('nombre_archivo', 'tamano_archivo', 'tipo_mime')
      `);
      ...
      if (!existingColumns.includes('nombre_archivo')) {
        await connection.execute(`
          ...
          ADD COLUMN nombre_archivo VARCHAR(255) AFTER estado
        `);
        console.log('  ✅ Agregada columna nombre_archivo');
      }
