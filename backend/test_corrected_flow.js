...
async function testCorrectedFlow() {
  try {
    ...
    console.log('🧪 Probando flujo corregido (sin llamar a siguiente-hoja)...');
    ...
    // 2. Crear hoja principal (sin llamar a siguiente-hoja)
    console.log('\n📋 Paso 2: Creando hoja principal...');
    const hojaData = {
      id_plataforma: 'YANGO',
      id_piloto: 4,
      id_vehiculo: 1,
      placa_id: 'TEST123',
      lectura_km_num: 1000,
      observaciones: 'Flujo corregido - sin siguiente-hoja',
    ...
    // 3. Agregar items revisados
    console.log('\n📋 Paso 3: Agregando items revisados...');
    const items = [
    ...
    // 4. Subir fotos de motocicleta
    console.log('\n📋 Paso 4: Subiendo fotos de motocicleta...');
    ...
    // 5. Subir fotos de items
    console.log('\n📋 Paso 5: Subiendo fotos de items...');
    for (const item of items) {
      try {
    ...
    console.log('\n✅ Flujo corregido completado exitosamente');
    
  } catch (error) {
    console.error('Error en el flujo corregido:', error.response?.data || error.message);
  }
}
